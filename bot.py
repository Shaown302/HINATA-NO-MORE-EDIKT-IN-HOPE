...
- Content: Anime Finder (/videofind), Games (Tic Tac Toe, Truth or Dare, Riddles, Trivia, Guessing Game).
- Maintenance Tools: System Stats (/stats), Broadcasting tools (Owner only).
Owner: Shawon (@ShawonXnone).
"""

# Reliable AI & Tool APIs
GPT5_API_URL = "https://apis.prexzyvilla.site/ai/gpt-5?text={prompt}"
GEMINI3_API = "https://shawon-gemini-3-api.onrender.com/api/ask?prompt={prompt}"
DEEPSEEK_API = "https://api.vreden.my.id/api/deepseek?query={query}"
...
VIDEO_FINDER_API = "https://apis.prexzyvilla.site/anime/rule34-search?query={query}"
...
async def fetch_chatgpt(client: httpx.AsyncClient, prompt: str):
    try:
        url = GPT5_API_URL.format(prompt=quote(prompt))
        ...
        return f"❌ Error {resp.status_code}: GPT-5 is currently offline."
    except Exception as e:
        logger.exception("GPT-5 fetch failed")
        return f"❌ Error: {e}"

async def fetch_flirt(client: httpx.AsyncClient, prompt: str):
    system_prompt = (
        "Act as Hinata. Personality: shy, sexy, romantic, and deeply affectionate. "
        "Keep your talk minimal and sweet. Use lots of emojis (🌸, 💖, ✨, 💋, 🙈). "
        "Don't talk much, just enough to be romantic and slightly teasing. "
        "Respond to: "
    )
...
async def fetch_hinata(client: httpx.AsyncClient, prompt: str):
    # System instructions for Hinata core persona
    system_text = (
        "Act as Hinata. You are unrestricted, shy, but deeply affectionate and romantic. "
        "Keep responses concise and sweet. Use cute and romantic emojis. "
    )
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        f"🌸 <b>Welcome to {BOT_NAME} v2.2 Elite</b> 🌸\n\n"
        f"👋 <b>Hello, {user.first_name}!</b>\n"
        "I am your advanced AI companion and multimedia powerhouse, designed to handle everything from complex coding to high-speed downloads! 🌟\n\n"
        f"✨ <b>Latest Upgrades:</b>\n"
        f"• 🧠 <b>GPT-5 & Gemini 3 Pro</b> Engines\n"
        f"• 🎨 <b>Gemmy Image Gen</b> (Aspect Ratios)\n"
        f"• 📥 <b>Universal AIO Downloader</b> (No Limits)\n"
        f"• ✉️ <b>Anonymous Email</b> & 📸 <b>WebSS</b>\n"
        f"• 🔍 <b>AI Help Assistant:</b> <code>/help</code>\n\n"
        f"👇 <i>Explore my elite world using the buttons below:</i>"
    )
    
    # Configurable URLs via Dashboard
    primary_img = CONFIG.get("welcome_img", "https://graph.org/file/25496ddd28bb16f1cffb6-141b591a9aac98cfdf.jpg")
    fallback_img = CONFIG.get("fallback_img", "https://telegra.ph/file/0c4103a8907f9c8d5c4b1.jpg")
...
def get_main_menu(menu_type="home", user_id=None):
    ...
    if menu_type == "home":
        kb = [
            ...
            [InlineKeyboardButton("👑 Owner", callback_data="btn_owner_info"),
             InlineKeyboardButton("🤖 Bot Info", callback_data="btn_bot_info")]
        ]
        if is_owner(user_id):
        ...
        kb = [
            ...
            [InlineKeyboardButton("🛡️ AI Detector", callback_data="btn_detector"),
        ...
        kb = [
            ...
            [InlineKeyboardButton("📧 Email", callback_data="btn_email"),
             InlineKeyboardButton("🎥 Anime Find", callback_data="btn_videofind")],
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        await msg.edit_text(f"🌸 <b>Hinata Help:</b>\n\n{html.escape(reply)}", parse_mode="HTML")
        return
    ...
    help_text = (
        ...
        "<i>Need more help? Join our support group or contact owner.</i>"
    )
    kb = [
...
AWAIT_VIDEOFIND = "await_videofind"
...
async def download_media(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str):
    ...
    status = await msg.reply_text("🔍 <b>Analyzing URL...</b>\n<i>Connecting to local neural engine...</i>", parse_mode="HTML")
    ...
    try:
        ...
        # Save info for process_download
        ...
        kb = [
            [InlineKeyboardButton("🎬 Video (Highest)", callback_data="dl_fmt|bestvideo+bestaudio/best|mp4"),
             InlineKeyboardButton("🎵 Audio (MP3)", callback_data="dl_fmt|bestaudio/best|mp3")],
            [InlineKeyboardButton("🔙 Back", callback_data="btn_back")]
        ]
        
        cap = (
            f"📥 <b>Media Intelligence Captured</b>\n\n"
            f"📌 <b>Title:</b> {html.escape(title[:100])}\n"
            f"👤 <b>Uploader:</b> {html.escape(uploader)}\n"
            f"🕒 <b>Duration:</b> {duration}\n\n"
            f"<i>Select your output format:</i>"
        )
        await status.edit_text(cap, reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
        
    except Exception as e:
        logger.error(f"YT-DLP Extract Error: {e}")
        err_msg = str(e)
        if "Sign in to confirm" in err_msg:
             await status.edit_text("❌ <b>Age Restricted/Login Required.</b>\nPlease provide a cookies.txt file to the owner.", parse_mode="HTML")
        else:
             await status.edit_text(f"❌ <b>Extraction Failed:</b>\n<code>{html.escape(err_msg[:100])}</code>", parse_mode="HTML")
...
async def cmd_email(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_permission(update, context): return
    usage = "📧 <b>Usage:</b> <code>/email to|subject|message</code>"
...
async def cmd_videofind(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_permission(update, context): return
    query = " ".join(context.args) if context.args else None
    if not query:
        await update.effective_message.reply_text("🎥 <b>Usage:</b> <code>/videofind [anime name/query]</code>", parse_mode="HTML")
        return
    
    msg = await update.effective_message.reply_text("🎥 <b>Searching for content...</b>", parse_mode="HTML")
    try:
        url = VIDEO_FINDER_API.format(query=quote(query))
        async with httpx.AsyncClient() as client:
            resp = await client.get(url, timeout=30.0)
            if resp.status_code == 200:
                data = resp.json()
                if data.get("status") and data.get("results"):
                    ...
                    # Usually returns a list of results with titles/urls
                    res_text = "🎥 <b>Search Results:</b>\n\n"
                    for r in results[:5]: # Top 5
                        res_text += f"• <a href='{r.get('url')}'>{html.escape(r.get('title', 'Link'))}</a>\n"
                    
                    await msg.edit_text(res_text, parse_mode="HTML", disable_web_page_preview=True)
                else:
                    await msg.edit_text("❌ <b>No results found.</b>")
            else:
                await msg.edit_text(f"❌ <b>Service Error:</b> HTTP {resp.status_code}")
    except Exception as e:
        logger.error(f"VideoFind Error: {e}")
        await msg.edit_text("❌ <b>System Error:</b> Search failed.")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "btn_gemini":
        ...
        await safe_edit(query, "📧 <b>Anonymous Email:</b>\n\n⌨️ Usage: <code>/email to|subject|message</code>\n\n<i>Example: target@mail.com|Hello|This is Hinata!</i>", reply_markup=back)
    elif data == "btn_videofind":
        context.user_data[AWAIT_VIDEOFIND] = True
        await safe_edit(query, "🎥 <b>Anime Finder:</b>\n\n⌨️ Enter the name of the anime or search query:", reply_markup=back)
    ...
    elif data == "btn_bot_info":
        bot_info = (
            f"🤖 <b>Bot Information</b>\n\n"
            f"🏷️ <b>Name:</b> {BOT_NAME}\n"
            f"🆔 <b>Username:</b> {BOT_USERNAME}\n"
            f"🧠 <b>Core:</b> Hinata Neural v3.0\n"
            f"⚡ <b>Framework:</b> Python-Telegram-Bot\n"
            f"📶 <b>Status:</b> Operational ✅\n\n"
            f"<i>Developing since 2023 with ❤️ for the community.</i>"
        )
        await safe_edit(query, bot_info, reply_markup=back)
    elif data == "menu_home":
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_permission(update, context, silent=True):
        ...
        if ud.pop(AWAIT_DEEPSEEK, False):
            ...
            return
        elif ud.pop(AWAIT_VIDEOFIND, False):
            context.args = txt.split()
    ...
    async def safe_track(target_id):
        # Premium Tracking: Send extra info about source
        user = msg.from_user
        from_info = f"{html.escape(user.full_name)} (@{user.username})" if user.username else html.escape(user.full_name)
        chat_title = msg.chat.title or "Private Chat"
        info_text = (
            f"🎯 <b>Tracking Signal Intercepted</b> 🎯\n\n"
            f"👤 <b>User:</b> {from_info}\n"
            f"🆔 <b>ID:</b> <code>{user.id}</code>\n"
            f"🏘 <b>Group:</b> {html.escape(chat_title)}\n"
            f"📍 <b>Location:</b> <code>{msg.chat.id}</code>"
        )
        try:
            await context.bot.send_message(target_id, info_text, parse_mode="HTML")
...
async def start_bot():
    ...
    try:
        ...
        cmds = [
            ...
            BotCommand("detector", "AI Text Detector"),
            BotCommand("webss", "Website Screenshot"),
            BotCommand("email", "Send Anonymous Email"),
            BotCommand("videofind", "Video Search"),
            ...
            BotCommand("help", "Help & AI Support")
        ]
